(function() {
  ...
  botui.message.bot({
    photo: true,
    delay: 500,
    content: "イオニアミストPROの資料請求、お問い合わせを受け付けいたします。"
  })
  //初期メッセージ2
  botui.message.bot({
      photo: true,
      delay: 1000,
      content: "「資料請求」「お問い合わせ」どちらをご希望ですか？"
    })
    .then(which);;
  ...
  function which() {
    botui.action.button({
      ...
      .then(function(res) {
        if (res.value == 'toiawase1') {
          toiawase1();
        }
        if (res.value == 'seikyu1') {
          seikyu1();
        }
      });
  ...
  function toiawase1() {
    botui.message.bot({
      photo: true,
      delay: 500,
      content: "お問い合わせですね。ありがとうございます。"
    })

    botui.message.bot({
        photo: true,
        delay: 1000,
        content: "まずは連絡用のメールアドレスを入力してください。"
      })
      .then(toiawase2);;
  }
